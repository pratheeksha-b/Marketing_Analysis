...
select distinct(Company),ROI from market
...
#Compare each campaign ROI with previous campaign.
...
select Company,Channel_Used,count(*) as total_counts from market
group by Company,Channel_Used
order by Company asc,total_counts desc;
...
DELIMITER //
CREATE PROCEDURE Campaign_Types(Campaign varchar(50))
BEGIN
